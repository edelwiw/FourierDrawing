...
class Drawing(ZoomedScene):
    ...
    def construct(self):
        ...
        coefficients.sort(key=lambda x: np.sqrt(x[0].real ** 2 + x[0].imag ** 2), reverse=True)
        ...
        trace = TracedPath(components[-1][1].get_end, stroke_color=BLUE, stroke_width=5, dissipating_time=4.5, stroke_opacity=[0.5, 1])
        self.add(trace)

        # # zoom 
        # # self.wait(3)
        # self.zoomed_camera.frame.add_updater(lambda m: m.move_to(components[-1][1].get_end()))
        # self.zoomed_display.display_frame.set_color(PURPLE)
