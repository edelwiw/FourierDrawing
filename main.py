...
scale = 0.024
time_scale = 20
number_of_points = 10000 # number of points to calculate scalar product
number_of_components = 200 # number of components to use in fourier series
...
def points_decomposition(file, scale, number_of_points, number_of_components):
    f = open(file, 'r')
    points = f.read().split(';')
    points = [points[i].split(',') for i in range(len(points))]

    points_x = [float(points[i][0]) for i in range(len(points))]
    points_y = [float(points[i][1]) for i in range(len(points))]

    tck, u = interpolate.splprep([points_x, points_y], s=0)

    def path_func(t):
        out = interpolate.splev(t, tck) 
        return out[0] * scale + 1j * out[1] * scale
...
class Drawing(ZoomedScene):
    def construct(self):
        ...
        coefficients = points_decomposition('PEREGUDIN.txt', scale, number_of_points, number_of_components)

        plt.figure()
        fourier_func = fourier_exp_func(coefficients, number_of_components)
        x = np.linspace(0, 1, number_of_points)
        plt.plot(fourier_func(x).real, -fourier_func(x).imag)
        plt.show()
        ...
        trace = TracedPath(components[-1][1].get_end, stroke_color=BLUE, stroke_width=5,)# dissipating_time=20)
        self.add(trace)
